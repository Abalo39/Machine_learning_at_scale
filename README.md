...
### Optimization
We minimize the regularized squared error using **Alternating Least Squares (ALS)**. Our implementation achieved rapid convergence within **15 iterations**, stabilizing at a training RMSE of ~0.835 and a test RMSE of ~0.875.
...
2. **Setup**: Ensure you have Python installed with libraries such as `pandas`, `numpy`, `matplotlib`, and `torch` (or `tensorflow` depending on your NCF implementation).
3. **Run**: Open the `.ipynb` files in Jupyter or Google Colab to reproduce the figures and results.
